...
//================================================================================================================
Double_t GetYRangeExtremaFromList(TList* list, Bool_t doParticleSpectra, Bool_t returnMax) {
...
//       xMin                                  = 0.01;
    ...
    // get yRange
    Double_t yMin                           = GetYRangeExtremaFromList(list, kTRUE, kFALSE) * 0.5;
    Double_t yMax                           = GetYRangeExtremaFromList(list, kTRUE, kTRUE) * 2;
...
void ProduceParticleSpectraPlotFromListOnlyFinal(   TList* list,
    ...
    TLatex *labelALICE                     = new TLatex(0.94, 0.885, "ALICE");
...
void ProduceSingleParticleSpectraPlotFromListOnlyFinalWithFits( TList* list,
    ...
    // get yRange
    Double_t yMin                           = GetYRangeExtremaFromList(list, kTRUE, kFALSE) * 0.5;
    Double_t yMax                           = GetYRangeExtremaFromList(list, kTRUE, kTRUE) * 4;
    if (xLogPlot && collSys.Contains("Pb"))
        yMax                                = yMax*2;
...
//                 cout << "accepted: "<< tempName.Data() << endl;
    ...
    if (xLogPlot ){
        ...
        legend                              = GetAndSetLegend(0.20, 0.08, 0.41, 0.08+(1.05*0.04*nRows), "" ,1,0.28,0.04);
    }
    TLatex *labelEnergy                     = new TLatex(0.94, 0.925, Form("%s %s, %s", cent.Data(), collSys.Data(), energy.Data()));
    SetStyleTLatex( labelEnergy, 0.04,4, 1, 42, kTRUE, 31);
    TLatex *labelALICE                     = new TLatex(0.94, 0.885, "ALICE");
...
void ProduceParticleRatioPlotFromList(TList* list, TString collSys, TString energy, TString cent, TString suffix) {
    ...
    // create legend
    TLegend* legend                                     = GetAndSetLegend(0.4, 0.92-(0.05*nRows), 0.9, 0.92, Form("%s %s, %s", cent.Data(), collSys.Data(), energy.Data()));
...
//                     cout << "found " << Form("%sTo%s%sStat", fParticle[i].Data(), fParticle[k].Data(), fMethod[ii].Data()) << endl;
                    if (tempClass.Contains("TH1")) {
                        ...
                        if (tempHist) {
                            DrawMarker(tempHist, markers[j], markerSize[j], paint[j],paint[j]);
                            tempHist->Draw("e1,same");
                            legend->AddEntry(tempHist, Form("%s/%s %s", fRatioParticleLatex[i].Data(), fRatioParticleLatex[k].Data(), fMethod[ii].Data()), "lp");
                            j++;
                        }
                    } else if (tempClass.CompareTo("TGraph") == 0) {
                        ...
                        if (tempGraph) {
                            DrawMarker(tempGraph, markers[j], markerSize[j], paint[j],paint[j]);
                            tempGraph->Draw("p");
                            legend->AddEntry(tempGraph, Form("%s/%s %s", fRatioParticleLatex[i].Data(), fRatioParticleLatex[k].Data(), fMethod[ii].Data()), "lp");
                            j++;
                        }
                    } else if (tempClass.CompareTo("TGraphErrors") == 0) {
                        ...
                        if (tempGraphErrs) {
                            ...
                            legend->AddEntry(tempGraphErrs, Form("%s/%s %s", fRatioParticleLatex[i].Data(), fRatioParticleLatex[k].Data(), fMethod[ii].Data()), "lp");
                            j++;
                        }
                    } else if (tempClass.CompareTo("TGraphAsymmErrors") == 0) {
                        ...
                        if (tempGraphAsymErrs) {
                            ...
                            legend->AddEntry(tempGraphAsymErrs, Form("%s/%s %s", fRatioParticleLatex[i].Data(), fRatioParticleLatex[k].Data(), fMethod[ii].Data()), "lp");
                            j++;
                        }
                    }
                }
...
//Function to produce plot containing all particle ratio in list
//================================================================================================================
void ProduceParticleRatioPlotFromListOnlyFinal(TList* list, TString collSys, TString energy, TString cent, TString suffix, Bool_t xyLogPlot = kFALSE) {
    ...
    // get xRange
    Double_t xMin                                       = 0;
    Double_t xMax                                       = GetXRangeExtremaFromList(list, kTRUE, kTRUE);
    if(xyLogPlot){
        xMin                                            = GetXRangeExtremaFromList(list, kTRUE, kFALSE) * 0.5;
        xMax                                            = xMax * 1.3;
    }
    ...
    Double_t yMax                                       = 2;
    if(xyLogPlot){
        yMin                                            = GetYRangeExtremaFromList(list, kTRUE, kFALSE) * 0.5;
        if(yMin<0.001)
            yMin                                        = 0.0015;
        yMax                                            = GetYRangeExtremaFromList(list, kTRUE, kTRUE) * 2;
    }
    ...
    // create legend
    TLegend* legend                         = GetAndSetLegend(0.12, 0.94, 0.59, 0.94-(1.05*0.05*nRows), "" ,2,0.14,0.04);
    TLatex *labelEnergy                     = new TLatex(0.94, 0.91, Form("%s %s, %s", cent.Data(), collSys.Data(), energy.Data()));
    SetStyleTLatex( labelEnergy, 0.04,4, 1, 42, kTRUE, 31);
    TLatex *labelALICE                     = new TLatex(0.94, 0.87, "ALICE");
    SetStyleTLatex( labelALICE, 0.04,4, 1, 42, kTRUE, 31);
...
//                     cout << GetParticleMarkerStyle(fParticle[i]) << "\t" << GetParticleMarkerSize(fParticle[j]) << "\t" << GetParticleColor(fParticle[i]) << endl;
    ...
    // save canvas
    if (histCounter) canvas->SaveAs(Form("%s/ParticleRatios_Final.%s", outputDir.Data(), suffix.Data()));
...
void ProducePlotWithRatioToFit( TH1D* histo1,
    ...
    tempList->Add(histo2);
    tempList->Add(histo3);
    Double_t yMin                           = GetYRangeExtremaFromList(tempList, kFALSE, kFALSE) * 0.2;
    Double_t yMax                           = GetYRangeExtremaFromList(tempList, kFALSE, kTRUE) * 2.5;
    ...
    SetHistogramm(histo1, xTitle, yTitle, yMin, yMax);
    ...
    else legendHisto                        = GetAndSetLegend(0.15, 0.125, 4, 1, Form("%s %s, %s", cent.Data(), collSys.Data(), energy.Data()));
    legendHisto->SetBorderSize(0);
    ...
    if (isRatioPlot) legendFit              = GetAndSetLegend(0.625, 0.65, 6, 1);
    else legendFit                          = GetAndSetLegend(0.625, 0.65, 6, 1);
    ...
    histoRatioToFit1->GetYaxis()->SetRangeUser(yMinPlotRatio,yMaxPlotRatio);

    if (xMinPlot != -1000 && xMaxPlot != -1000)
        histoRatioToFit1->GetXaxis()->SetRangeUser(xMinPlot, xMaxPlot);

    DrawMarker(histoRatioToFit1, 20, 1.5, kBlack, kBlack);
    ...
    if (xMinPlot != -1000 && xMaxPlot != -1000){
        DrawLine(xMinPlot, xMaxPlot, 1, 1, 1, kGray+2, 2);
        DrawLine(xMinPlot, xMaxPlot, 1.1, 1.1, 1, kGray+2, 3);
        DrawLine(xMinPlot, xMaxPlot, 0.9, 0.9, 1, kGray+2, 3);
    } else {
...
void ProducePlotWithRatioToFit( TGraph* graph1,
    ...
    TH1D* dummyHisto                        = new TH1D("dummyHisto", "", 100000, xMin, xMax);
    SetHistogramm(dummyHisto, xTitle, yTitle, 0.8*yMin, 1.2*yMax, 0.7, 0.92, 0.053, kFALSE);
    ...
    else legendGraph                        = GetAndSetLegend(0.15, 0.125, 4, 1, Form("%s %s, %s", cent.Data(), collSys.Data(), energy.Data()));
                                                ...
                                                TString paramDirName  )
{
    ...
    // get yRange
    //~ Double_t yMin                           = GetYRangeExtremaFromList(inputlist, kTRUE, kFALSE) * 0.5;
    Double_t yMin                           = 4e-14;
    Double_t yMax                           = GetYRangeExtremaFromList(inputlist, kTRUE, kTRUE) * 4;
