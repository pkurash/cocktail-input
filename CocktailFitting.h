...
void WriteParametrizationsFile(TList* list, TList* inputList, TString uniqueName, Int_t systSwitch = 0, TString suffix = "pdf") {
    ...
    if(systSwitch == 0)
        PlotCocktailParametrizationListFullRange(parametrizationList,inputList,collSys,energy,cent,suffix,paramDirName);

    // create output file containing only parametrizations
    TFile* paramFile                                            = new TFile(Form("parametrizations/%s.root", collSys.Data()), "UPDATE");
    paramFile->cd();
