...
//     xSection = 2.09 barn (DMeson paper)
    ...
    if (includeParticle[0] || includeParticle[1]) {
        if (enable5TeV) {
            ...
            fileNeutralMeson5TeVMBNew                           = new TFile("pPb/5_TeV/CombinedResultsPaperPPb5023GeV_2017_07_26.root");
        }
    }
                    ...
                    if (i==0 || i == 16) {
                        ...
                        if (!useOldMBFile) {
                            if (fileNeutralMeson5TeVMBNew->GetListOfKeys()->Contains("Pi0pPb_5.023TeV")) {
                                ...
                                TString tempMethod[6]                       = {"PCM", "PHOS", "EMCAL", "PCMEMCAL", "Dalitz", "Comb"};
                                TString tempReadMethod[6]                   = {"PCM", "PHOS", "EMC", "PCM-EMC", "PCM-Dal", "Comb"};
                                // spectra
                                for (Int_t j=0; j<6; j++) {
                                    ...
                                    if (tempList->Get(Form("%sStatErr", tempName.Data())) && tempList->Get(Form("%sSysErr", tempName.Data()))) {
                                        ...
                                        else if (j==5)  tempMethodName      = fMethod[1];   // Comb
                    ...
                    if (i==0 || i == 16) {
                        ...
                        if (!useOldMBFile) {
                            if (fileNeutralMeson5TeVMBNew->GetListOfKeys()->Contains("EtapPb_5.023TeV")) {
                                ...
                                TString tempMethod[4]                       = {"PCM", "EMCAL", "PCMEMCAL", "Comb"};
                                TString tempReadMethod[4]                   = {"PCM", "EMC", "PCM-EMC", "Comb"};
                                ...
                                for (Int_t j=0; j<4; j++) {
                                    ...
                                    if (tempList->Get(Form("%sStatErr", tempName.Data())) && tempList->Get(Form("%sSysErr", tempName.Data()))) {
                                        ...
                                        else if (j==3)  tempMethodName      = fMethod[1];   // Comb
                                ...
                                for (Int_t j=0; j<4; j++) {
                                    ...
                                    if (tempList->Get(Form("%sRatioEtaToPi0%spPb5023GeVStatErr", tempName.Data(), tempMethod[j].Data())) && tempList->Get(Form("graphRatioEtaToPi0%spPb5023GeVSysErr", tempMethod[j].Data()))) {
                                        ...
                                        if (j!=3) {
                                            ...
                                            SetHistoProperties(histoEtaToPi05TeVStat, Form("%sTo%s%sStat", fParticle[1].Data(), fParticle[0].Data(), tempMethodName.Data()), "#it{p}_{T} (GeV/#it{c})", "#eta / #pi^{0}", "");
                                            lists5TeV[i]->Add(histoEtaToPi05TeVStat);
                                        } else {
                                            graphEtaToPi05TeVStat           = (TGraphAsymmErrors*)tempList->Get(Form("graphRatioEtaToPi0%spPb5023GeVStatErr", tempMethod[j].Data()));
                                        ...
                                        graphEtaToPi05TeVSys                = (TGraphAsymmErrors*)tempList->Get(Form("graphRatioEtaToPi0%spPb5023GeVSysErr", tempMethod[j].Data()));
...
//         bIntegYieldPub[0][0]                    = kTRUE;
    ...
    ProduceSpectrumInCentralityBin(lists5TeV, fCentrality[5], fParticle[6], fParticle[5], "");
    ProduceSpectrumInCentralityBin(lists5TeV, fCentrality[5], fParticle[7], fParticle[5], "");
    ProduceSpectrumInCentralityBin(lists5TeV, fCentrality[5], fParticle[16], fParticle[15], "");
